...
**Stack:** Next.js 16 · PostgreSQL 16 · Docker · Caddy (HTTPS)  
...
Afya Bridge is deployed as a containerized application. The production stack consists of three services:
...
No Node.js, Nginx, or PostgreSQL installation is required on the host. Docker and Docker Compose are the only prerequisites.
...
This builds the containers and starts all services.
...
- All three containers (`db`, `app`, `caddy`) report status `running`.
...
| `POSTGRES_USER` | No | Default: `afya` |
| `POSTGRES_DB` | No | Default: `afya` |
...
Include the `uploads` and `pgdata` volumes in your backup strategy if uploaded media and database content must be preserved across server rebuilds.
...
# PostgreSQL shell
docker compose exec db psql -U afya -d afya
...
| Database connection errors | Run `docker compose restart db app` |
